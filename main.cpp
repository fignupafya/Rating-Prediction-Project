...
void max_pred_Heapify(temp_compare_obj arr[], int n, int i)
{
    ...
    if (l < n && arr[l].similarity < arr[smallest].similarity)
        smallest = l;

    // If right child is larger than largest so far
    if (r < n && arr[r].similarity  < arr[smallest].similarity)
        smallest = r;
    ...
    if (smallest != i) {
        ...
        // Recursively heapify the affected sub-tree
        max_pred_Heapify(arr, n, smallest);
    }
}
// main function to do heap sort
void max_pred_Heap_Sort(temp_compare_obj arr[], int n)
{
    // Build heap (rearrange array)
    for (int i = n / 2 - 1; i >= 0; i--)
        max_pred_Heapify(arr, n, i);
    // One by one extract an element from heap
    for (int i = n - 1; i >= 0; i--) {
...
struct similarity_output  calc_cosine(int question_userid,int current_userid,int* item_ids,int id_array_size){
    struct similarity_output output{0,0};
    ...
    for (int i=0;i<id_array_size;i++){
        ...
        if (c_rat!=-1){
            ...
            if (c_rat == 0) c_rat += 0.00001;
            if (q_rat == 0) q_rat += 0.00001;
            num += c_rat * q_rat;
            denom_q += q_rat * q_rat;
            denom_c += c_rat * c_rat;
    ...
    if (common!=0) {
        denom_c = sqrt(denom_c);
        denom_q = sqrt(denom_q);
        output.similarity = num / (denom_q * denom_c);
        output.common = common;
    }

    if (isnan(output.similarity)) {
        cout<<"ALERT!\n";
        cout<< num << " - " << denom_q << " - " << denom_c << " - " << common << " - " << id_array_size <<"\n"<<endl;
        output.similarity = 1;}
...
void predict(int qid, int userid, int itemid){
    ...
    temp_compare_obj* cosine_results = new temp_compare_obj[user_array_size];

    a = 0;
    b = 0;
    //tüm userlar üzerinde gezinip filmi izleyenlerin cosineını depolama
    while(a!=-1){

        //baktığımız user aranan filmi izlemiş mi?
        double current_rating = userlist.array[a].rating_array_class.array[itemid].rate;
        if (current_rating != -1){
            //cosineını arraye ekle

            //for cosine
//            struct similarity_output output = calc_cosine(userid,a,item_ids,id_arr_len);

            //for Manhattan
            struct similarity_output output = calc_Manhattan(userid,a,item_ids,id_arr_len);


            cosine_results[b].set(output.similarity, output.common, current_rating);
            b++;
        }
...
//    max_pred_Heap_Sort(cosine_results , b);
    min_pred_heap_Sort(cosine_results,b);
    ...
    for (int i=0; i<b; i++){
        ...
        result += cosine_results[i].rating * cosine_results[i].common;
        common_total += cosine_results[i].common;
    }
